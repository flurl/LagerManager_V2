<template>
  <div>
    <v-row class="mb-2" align="center">
      <v-col><h2>Liefereinheiten</h2></v-col>
      <v-col cols="auto">
    ...
    <v-dialog v-model="dialog" max-width="400">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-text-field v-model.number="form.quantity" label="Menge" type="number" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
const form = ref({ name: '', quantity: 1 })
...
async function fetchItems() {
  loading.value = true
  try {
    const res = await api.get('/delivery-units/')
...
function openNew() { form.value = { name: '', quantity: 1 }; dialog.value = true }
function openEdit(item) { form.value = { ...item }; dialog.value = true }

async function save() {
  if (form.value.id) {
    await api.put(`/delivery-units/${form.value.id}/`, form.value)
  } else {
    await api.post('/delivery-units/', form.value)
  }
  dialog.value = false
  await fetchItems()
}
...
async function deleteItem(item) {
  ...
  await api.delete(`/delivery-units/${item.id}/`)
  await fetchItems()
}
